...
<title>Your Give 'n Take Agent — Private AI Matchmaking on Sui</title>
<style>
  :root {
    --bg: #0a0e1a;
    --surface: #111827;
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .badge { font-size: 15px; padding: 5px 14px; border-radius: 20px; border: 1px solid var(--border); color: var(--text2); }
  ...
  .data-card.old-world .rating-num { font-size: 56px; font-weight: 700; color: #7a7a7a; text-align: center; padding: 18px 0 12px; font-family: monospace; }
...
<div class="container">
  <div class="header">
    ...
    <p class="tagline" style="font-size:23px;font-weight:600;line-height:1.5;">How do you really feel about the people in your community?<br>Your agent is yours — you control it. Your data is private — you own it.<br>It negotiates your perfect matches, and never shares a word.</p>
    <div class="badges">
  ...
  <div id="data-compare-section">
    <div class="data-compare">
      <div class="data-card old-world">
        ...
        <div class="thats-it">Every person in your community reduced to one number.</div>
      </div>
      <div class="compare-arrow" style="flex-direction:column;gap:6px;text-align:center">\u2192<span style="font-size:15px;font-weight:600;color:var(--accent2)">Let's change that.</span></div>
...
<!-- Roadmap button -->
<button class="roadmap-btn" onclick="document.getElementById('roadmap-overlay').classList.add('visible')">\u{1F4CB} Product Roadmap</button>
...
<div class="roadmap-overlay" id="roadmap-overlay" onclick="if(event.target===this)this.classList.remove('visible')">
  <div class="roadmap-panel">
    ...
    <div class="phase">
      <div class="phase-label active-phase">\u25CF Phase 1: Krista's Agent</div>
      <p>Krista gets a private agent she can pour everything into. Player observations. Group chemistry notes. Who to keep apart. The agent stores it all, encrypted and searchable. Every time she builds a group, the system records her decision. Every edit to an AI suggestion is training data. Adoption rate is the metric: what percentage of AI suggestions does Krista use as-is, edit, or discard? That number going up is the proof the system is learning.</p>
      <p><strong>The trust loop:</strong> Krista tries a suggestion. It works. She tries another. It works better. She starts trusting the easy ones and focusing on the hard ones. Her workload drops while quality stays the same or improves.</p>
    </div>

    <div class="phase">
      <div class="phase-label future-phase">\u25CB Phase 2: Expert Staff Get Agents</div>
      <p>The pros, coordinators, and front desk staff each get their own agent scoped to their expertise. A teaching pro notices skill nuances. A front desk staffer notices who arrives together. Each observation enriches the community graph. The AI now learns from multiple expert perspectives, not just one.</p>
    </div>

    <div class="phase">
      <div class="phase-label future-phase">\u25CB Phase 3: Every Player Gets Their Own Agent</div>
      <p>A personalized invitation feed. Sessions the agent thinks you will love. Private encrypted conversations where you tell the agent who you love playing with, who drained you, what you want more of. Because privacy is guaranteed by Sui cryptography, players can be brutally honest. The system coordinates around their honest preferences without ever exposing them.</p>
      <p><strong>The flywheel:</strong> A few players try it. Better matches appear. Others notice and want in. Trust builds through experience. Adoption compounds because the experience is jaw-dropping.</p>
    </div>

    <div class="phase">
      <div class="phase-label future-phase">\u25CB Phase 4: Multi-Club Expansion</div>
      <p>Every correction from every coordinator at every club trains the same system. New clubs adopt intelligence that already understands great group composition, seeded by patterns Krista taught it. Coordinator dependency dissolves as intelligence scales. Contribution history becomes portable across clubs via on-chain reputation.</p>
    </div>

    <p style="margin-top:14px;font-size:11px;color:var(--text2);">NEPC first \u2192 pickleball \u2192 social sports \u2192 every real-world community built on human value exchange.</p>
  </div>
</div>
...
function startConversation() {
  ...
  fetch('/api/start', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) })
    ...
    .then(function(data) {
      if (data.error) { showError(data.error); btn.disabled = false; btn.textContent = 'Start Conversation'; return; }
      ...
      document.getElementById('data-compare-section').style.display = 'none';
    ...
    .catch(function(e) { showError('Failed to start: ' + e.message); btn.disabled = false; btn.textContent = 'Start Conversation'; });
}
